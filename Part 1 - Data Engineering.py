...
aggregate_sensors = (
  spark.readStream.format('delta').table("manufacturing.sensors_raw")          # ソースのDeltaテーブルからデータをストリームとして読み込む
    ...
    .agg(
      F.avg('temperature').alias('temperature'), F.avg('humidity').alias('humidity'),
      F.avg('pressure').alias('pressure'), F.avg('moisture').alias('moisture'),
      F.avg('oxygen').alias('oxygen'),F.avg('radiation').alias('radiation'),
      F.avg('conductivity').alias('conductivity'))
