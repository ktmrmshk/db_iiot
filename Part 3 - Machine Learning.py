...
# MAGIC ## Part 3 - 機械学習
# MAGIC 
# MAGIC このノートブックでは、Azure上でIIoT(Industrial IoT)のIngest、Processing、Analyticsを行うための以下のアーキテクチャをデモしています。デモでは以下のようなアーキテクチャが実装されています。
...
spark.conf.set(f"fs.azure.account.key.{storage_account}.dfs.core.windows.net", dbutils.secrets.get("lab","adls_key"))
...
def train_distributed_xgb(readings_pd, label_col, prediction_col):
  ...
  with mlflow.start_run():
    ...
    # code修正
    feature_cols = readings_pd.drop(['facilityid', label_col, prediction_col], axis=1).columns
...
# MAGIC 
# MAGIC 1. `params` 変数に渡されたモデルパラメータ（alpha、colsample、learning rate など）
...
# APIを呼び出してデータを取得
# (MLflowやAzureMLのモデルURIに置き換えてください。)
temp_prediction_uri = "https://adb-xxxxxxxxxxxx.azuredatabricks.net/model/temperature_prediction/3/invocations" 
databricks_token = "xxxxxxxxxxxxxxxx" # あなたのDatabricks Personal Access Tokenに置き換えてください。

# RESTを使ってAPIを呼び出し、結果を返す関数を定義
...
# MAGIC %md 温度が25.2～25.6、圧力が34～35.4の場合、1分後には温度が安全動作閾値を外れて故障していることがわかります。
